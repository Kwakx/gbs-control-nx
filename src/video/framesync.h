...
#include "../config/Config.h" // For DEBUG_IN_PIN
#include "../wifi/WiFiManager.h" // For handleWiFi

// Forward declarations for globals used in this header
// These are defined in Globals.h/Globals.cpp, but we use forward declarations
// to avoid circular dependencies (Globals.h includes this file)
// However, for template class FrameSyncManager, we need full definitions
// So we include State.h which has the full struct definitions
#include "../core/State.h"
class Si5351mcu;
extern struct runTimeOptions *rto;
extern Si5351mcu Si;

// SerialMirror needs full definition for printf() calls
// Include it here since it doesn't create circular dependency
...
namespace MeasurePeriod {
    extern volatile uint32_t stopTime, startTime;
    extern volatile uint32_t armed;
    extern volatile uint32_t isrPrepareCount;
    extern volatile uint32_t isrMeasureCount;
